package org.muskacirca.finder.rs.controler;
...
public class UserControler {
    ...
    public EAppMessage isUserEligibleToSubscription(Session session, User user) {
        ...
        UserDAO existingUser = (UserDAO) databaseAdaptor.findOneByProperty(session, UserDAO.class, "name", user.getLogin());
        if (existingUser != null) {
            LOG.debug("User " + user.getLogin() + " already exist");
            return EAppMessage.LOGIN_ALREADY_EXIST;
        }

        existingUser = (UserDAO) databaseAdaptor.findOneByProperty(session, UserDAO.class, "mail", user.getEmail());
        if (existingUser != null) {
